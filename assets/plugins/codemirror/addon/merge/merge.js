...
(function(mod) {
  ...
  function DiffView(mv, type) {
    ...
    this.classes = type == "left"
      ? {chunk: "CodeMirror-merge-l-chunk",
         ...
         conect: "CodeMirror-merge-l-conect"}
      : {chunk: "CodeMirror-merge-r-chunk",
         start: "CodeMirror-merge-r-chunk-start",
         end: "CodeMirror-merge-r-chunk-end",
         insert: "CodeMirror-merge-r-inserted",
         del: "CodeMirror-merge-r-deleted",
         conect: "CodeMirror-merge-r-conect"};
  }
  ...
  DiffView.prototype = {
    ...
    init: function(pane, orig, options) {
      ...
      if (this.mv.options.conect == "align") {
  ...
  function registerScroll(dv, otherDv) {
    dv.edit.on("scroll", function() {
      syncScroll(dv, true) && makeconections(dv);
    });
    dv.orig.on("scroll", function() {
      syncScroll(dv, false) && makeconections(dv);
      if (otherDv) syncScroll(otherDv, true) && makeconections(otherDv);
    });
  }
  ...
  function setScrollLock(dv, val, action) {
    dv.lockScroll = val;
    if (val && action != false) syncScroll(dv, DIFF_INSERT) && makeconections(dv);
  ...
  function makeconections(dv) {
    ...
    for (var i = 0; i < dv.chunks.length; i++) {
      ...
      if (ch.editFrom <= vpEdit.to && ch.editTo >= vpEdit.from &&
        ...
        drawconectorsForChunk(dv, ch, sTopOrig, sTopEdit, w);
    }
  }
  ...
  function drawconectorsForChunk(dv, chunk, sTopOrig, sTopEdit, w) {
  ...
  var MergeView = CodeMirror.MergeView = function(node, options) {
    ...
    var onResize = function() {
      if (left) makeconections(left);
      if (right) makeconections(right);
    };
